...
- group: the number of categories for each variable (The maximum number is 10 given the computation limitation)
...
# Reference: [1] Keil, Alexander P., et al. "A quantile-based g-computation approach to addressing the effects of exposure mixtures." Environmental health perspectives 128.4 (2020): 047004.
(estimate.amc.gcomp = qgcomp.noboot(y~., data=output, family="gaussian", q = NULL))
end.time1 <- Sys.time()
(time.amcgcomp <- end.time1 - start.time1)

# quantile-based g-computation
start.time2 <- Sys.time()
(estimate.quantile.gcomp = qgcomp.noboot(y~., data=data[[2]], family="gaussian", q = 4))
end.time2 <- Sys.time()
(time.quantile.gcomp <- end.time2 - start.time2)

# wqs
start.time3 <- Sys.time()
exposures = paste0("X",1:5)
(wqsfit  <- suppressWarnings(gwqs(y ~ wqs, mix_name=exposures, q=4, data = data[[2]], family = "gaussian")))
end.time3 <- Sys.time()
(time.wqs <- end.time3 - start.time3)
