...
#' @param min_num The minimum sample size of each category, which can be set as a positive integer. (default = 1) We recommend the number is larger than 5% of the overall sample size.
#'
#' @return The categorized data and the categorization thresholds
#' \item{output}{The categorized data}
#' \item{threshold}{The thresholds/cutoffs of the categorization for each variable}
#' @export
#'
#' @examples


amc = function(data, group = 2, min_num = 1) {
  output = data
  ...
  # Parameter check
  if (group>5 | group<2 | min_num<=0 | min_num>=1 | num_sub < (group * min_num))
  ...
  threshold <- apply(mat_threshold, 2, sort)

  returnlist <- list(output = output, threshold = threshold)
  return(returnlist)
}
